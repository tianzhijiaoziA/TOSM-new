...
def get_pc_radon_ADMM_TV_mri(sde, predictor, corrector, inverse_scaler, snr, mask=None,
    ...
    def get_update_fn_slicer(update_fn):
        def radon_update_fn(model, data, x, t):
            with torch.no_grad():
                vec_t = torch.ones(1, device=x.device) * t
                x, x_mean = update_fn(x, vec_t, model=model)
                return x, x_mean
    ...
    def data_fidelity(x, measurement=None):
      x = torch.real(ifft2(fft2(x) * (1. - mask) + measurement))
      x_mean=x
      return x, x_mean
    #get_update_fn_slicer高维正交
    predictor_denoise_update_fn = get_update_fn_slicer(predictor_update_fn)
    corrector_denoise_update_fn = get_update_fn_slicer(corrector_update_fn)
    mc_update_fn = get_ADMM_TV_fn()
    ...
    def pc_radon(model, data, measurement=None):
        with torch.no_grad():
            ...
            for i in tqdm(range(sde.N)):
                ...
                # 1. batchify into sizes that fit into the GPU
                #x_batch = batchfy(x, 40)
                x_batch_sing = x
                #print(x_batch_sing.shape)
                # 2. Run PC step for each batch
                x_agg = list()
                #for idx, x_batch_sing in enumerate(x_batch):
                # linshi = torch.zeros_like(x).to(x.device)
                ...
                # 5. Run  data_fidelity
                # if (i%2)==0:
                #     x, x_mean = data_fidelity(x, measurement=measurement)
                ...
                if (i % 10) == 0:
                    ...
                    print(x_mean.shape)
                    #plt.imsave(save_root / 'recon' / 'progress' / f'progress{i}.png', clear(x_mean[0:1]), cmap='gray')
                    #plt.imsave('./process_z'+str(i)+'.png', clear(x_mean[:,:,100,:]), cmap='gray')
                    io.savemat('./zhengshi_15_mri_our_x_mean{1200}_.mat',{'x_mean': clear(x_mean)})
            return inverse_scaler(x_mean if denoise else x)
