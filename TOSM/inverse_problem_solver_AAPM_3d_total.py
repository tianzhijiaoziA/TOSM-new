...
vol_name = 'L067'
root = Path(f'/opt/data/private/zhangzhx2/3D-SDE/AAPM/L067_test_ye')
...
if sde.lower() == 'vesde':
    ...
    ckpt_filename = f"/opt/data/private/zhangzhx2/3D-SDE/score_AAPM/workdir/AAPM512_3d_4slicer/checkpoints/checkpoint_{ckpt_num}.pth"
...
irl_types = ['input', 'recon', 'label', 'fbp', 'sinogram']
for t in irl_types:
    if t == 'recon':
...
#     count += 1
io.savemat(str(save_root / 'recon' / f'recon_3d.mat'),{'input': clear(fbp),'recon': clear(x),'label': clear(img)})
