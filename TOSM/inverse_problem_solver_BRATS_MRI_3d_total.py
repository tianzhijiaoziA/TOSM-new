...
sde = 'VESDE'
num_scales = 1200#1000的效果相当于2000，正式实验的时候可以使用2000
ckpt_num = 95
N = num_scales

root = './data'
vol = 'MRI/Brats18_CBICA_AAM_1/'
...
if sde.lower() == 'vesde':
  ...
  if config_name == 'fastmri_knee_320_ncsnpp_continuous':
    ckpt_filename = f"./ve/{config_name}/checkpoint_{ckpt_num}.pth"
...
all_img = []
##将图像补全为240*240
img_zeros = torch.zeros([85,1,240,240])
...
for lamb in lamb_list:
    for rho in rho_list:
        ...
        # Inference
        ###############################################
        #img是3D图像，通道数在（b,1,X,X）
        # forward model
        kspace = fft2(img)

        # generate mask
        npmask = np.load("./001zhibiaogaomask_15_0.15_gua1d.npy")
        npmask1 = cv2.resize(npmask, (240,240))
        mask = torch.from_numpy(npmask1)
        mask = mask.view(1,1,240,240)
        ...
        #io.savemat('./zhengshi_15_mri_x_mean{1500}.mat',{'input': clear(ifft2(under_kspace)),'label': clear(img)})
        count = 0 
