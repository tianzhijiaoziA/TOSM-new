...
class ReverseDiffusionPredictor(Predictor):
  ...
  def update_fn(self, x, t):
    f1 = torch.zeros_like(x).to(x.device)
    f2 = torch.zeros_like(x).to(x.device)
    f3 = torch.zeros_like(x).to(x.device)
    for j in range(x.shape[0]):
        x_pre = x[j,:,:,:].unsqueeze(0)
       
  
       
        grad_linshif1, G1 = self.rsde.discretize(x_pre, t)

  
        f1[j,:,:,:] = grad_linshif1[:, :, :, :].squeeze(0)
    for k in range(x.shape[0]):
      x_pre1 = x.permute(3,1,2,0)
      x_pre = x_pre1[k,:,:,:].unsqueeze(0)
      #x_pre = x_pre.repeat(1,1,1,16)
      #print(x_pre.shape)
      grad_linshif2, G2 = self.rsde.discretize(x_pre, t)
      
      f2[:,:,:,k] = (grad_linshif2[:, :, :, :].permute(3,1,2,0)).squeeze(3)
      #print(grad1.shape,grad2.shape)
      ##可以开启第三层
    # for l in range(x.shape[0]):
    #   x_pre1 = x.permute(2,1,0,3)
    #   x_pre = x_pre1[l,:,:,:].unsqueeze(0)
    #   #x_pre = x_pre.repeat(1,1,1,16)
    #   #print(x_pre.shape)
    #   grad_linshif3, G3 = self.rsde.discretize(x_pre, t)
      
    #   f3[:,:,l,:] = (grad_linshif3[:, :, :, :].permute(2,1,0,3)).squeeze(2)
    #   #print(grad1.shape,grad2.shape)
    
    #f, G = self.rsde.discretize(x, t)
    z1 = torch.randn_like(x)
    z2 = torch.randn_like(x)
    z3 = torch.randn_like(x)
    x_mean = x - 0.5*(f1+f2)
    x = x_mean + 0.5*(G1[:, None, None, None] * z1 + G2[:, None, None, None] * z2)
    return x, x_mean
...
class LangevinCorrector(Corrector):
  ...
  def update_fn(self, x, t):
    ...
    grad1 = torch.zeros_like(x).to(x.device)
    grad2 = torch.zeros_like(x).to(x.device)
    grad3 = torch.zeros_like(x).to(x.device)
    ...
    for i in range(n_steps):
      for j in range(x.shape[0]):
        ...
        #print(x_pre.shape)
        grad1[j,:,:,:] = score_fn(x_pre, t).squeeze(0)
      ...
      for k in range(x.shape[0]):
        ...
        #x_pre = x_pre.repeat(1,1,1,16)
        #print(x_pre.shape)
        grad_linshi = score_fn(x_pre, t).permute(3,1,2,0)
        
        grad2[:,:,:,k] = grad_linshi[:, :, :, :].squeeze(3)
        #print(grad1.shape,grad2.shape)
      # for l in range(x.shape[0]):
      #   x_pre1 = x.permute(2,1,0,3)
      #   x_pre = x_pre1[l,:,:,:].unsqueeze(0)
      #   #x_pre = x_pre.repeat(1,1,1,16)
      #   #print(x_pre.shape)
      #   grad_linshi = score_fn(x_pre, t).permute(2,1,0,3)
        
      #   grad3[:,:,l,:] = grad_linshi[:, :, :, :].squeeze(2)
      #   #print(grad1.shape,grad2.shape)
        
      noise = torch.randn_like(x)
      grad_norm = torch.norm(grad1.reshape(grad1.shape[0], -1), dim=-1).mean()
      noise_norm = torch.norm(noise.reshape(noise.shape[0], -1), dim=-1).mean()
      step_size = (target_snr * noise_norm / grad_norm) ** 2 * 2 * alpha
      x_mean = x + 0.5*step_size[:, None, None, None] * (grad1+grad2)
